...
// LocalStats is the exported read-only view of an account's local checkin
// history: streak / total days / total credits, plus whether a claim is already
// recorded for today (for the WebUI "今日签到" column).
func LocalStats(accountID string) (streak, totalDays, totalCredits int, claimedToday bool) {
	historyMu.Lock()
	defer historyMu.Unlock()
	...
	return streak, totalDays, totalCredits, claimedToday
}
...
// CheckinWithToken runs the campaigns checkin for a raw device token that is not
...
// Succeeded reports whether the checkin claimed a benefit or was already claimed.
func (r CheckinResult) Succeeded() bool {
	return r.Status == checkinStatusClaimed || r.Status == checkinStatusAlreadyClaimed
}
