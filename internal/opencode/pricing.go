...
// SetClientProvider supplies candidate HTTP clients for refresh attempts so the
// models.dev refresh can ride the gateway's proxy transports.
...
func (store *PricingStore) refreshAndLog(ctx context.Context) {
	if err := store.Refresh(ctx); err != nil {
		if store.logger != nil {
			store.logger.Warn("models.dev metadata refresh failed", "component", "opencode.models", "error", err)
		}
		return
	}
	if store.logger != nil {
		store.logger.Info("models.dev metadata refreshed", "component", "opencode.models", "models", store.Snapshot().Models)
	}
}
