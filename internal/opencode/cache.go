...
type modelCatalogCache struct {
	...
	Zen             []string                              `json:"zen"`
	Go              []string                              `json:"go"`
	NativeProtocols map[Tier]map[string]protocol.Protocol `json:"native_protocols"`
	Unsupported     map[Tier]map[string]bool              `json:"unsupported"`
	Metadata        map[Tier]map[string]Metadata          `json:"metadata,omitempty"`
}
...
// LoadCache installs a validated disk snapshot into a catalog.
...
func validateCatalogCapabilities(native map[Tier]map[string]protocol.Protocol, unsupported map[Tier]map[string]bool) error {
	for tier, protocols := range native {
		...
		for model, p := range protocols {
			if model == "" || !protocol.Valid(p) {
...
func cloneTierProtocols(source map[Tier]map[string]protocol.Protocol) map[Tier]map[string]protocol.Protocol {
	result := map[Tier]map[string]protocol.Protocol{TierZen: {}, TierGo: {}}
