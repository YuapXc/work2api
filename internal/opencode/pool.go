...
// anonymousPool gives the shared "public" credential an independent cooldown
// per proxy. Anonymous nodes are never rebound: changing proxy is the failover
// mechanism because Zen rate-limits them by IP.
...
// CursorFor returns a cursor whose first choice is stable for the affinity key
// (FNV-64a of the session id), keeping a conversation on the same upstream key.
